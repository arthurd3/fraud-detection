...
| **2 (research-promoted)** | **Reduce references.kdt below 159MB cgroup limit** | Onda 20 research found 163MB > 159MB → kernel evicts pages → 5-10ms page-fault tail. Confirms hipótese mmap eviction. Options: quantize fields, drop topBbox/topSlot, smaller tree | code change + rebuild (~30 min) | médio (mexer em .kdt requer KdTreeBuilder + ExactAgree check) | calib + preview |

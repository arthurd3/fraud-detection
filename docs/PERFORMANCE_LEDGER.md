...
> Last update: 2026-05-21 (post-Onda 22 H1+H2 rerank attack — local gates green; preview pending).
...
**OPEN** (queued):
- H6: branch-free top-5 insertion sort. ~5 inserts/query post-H2 × few ns; marginal.
...
End-to-end win é modesto (-4.5 %) porque rerank é <10 % do trabalho de CPU; bulk é prime+BBF+descend (~15 µs em 17 µs). Mas o rerank stage agora está praticamente exauridopara H6/H7 marginais.

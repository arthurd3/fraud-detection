...
> Last update: 2026-05-21 (post-Onda 22 H1 rerank `round4(query)` hoist — local gates green; preview pending).
...
**OPEN** (queued):
- **H2: early-exit by i16 bound** — skip candidates with `poolI16Sum[p] > peekSumFinalI16` BEFORE calling `sqDistDoubleLikeC`. Soundness already proven in `KdTree.poolRecord` doc (i16-5th double bound argument: `1/1e8 ≫ 2e-14` integer-gap). Requires `int[POOL_CAP] poolI16Sum` in **production** (not INSTR-gated; ~256 KB extra in `KdScratch`) + a comparison in the loop. Expected ~870 ns/query savings (86 % × ~13 ns/call after H1). Bit-identity proof via `ExactAgree` 54 100 0-div. **Next iteration.**
- H6: branch-free top-5 insertion sort. ~13 inserts/query × few ns; marginal vs H2.

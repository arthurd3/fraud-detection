...
>
> ✅ **As‑built 2026‑05‑18 (Wave 6 — validated).** The honest exception above is **eliminated**. `HnswIndex.takeTop5` no longer does `new int[n]` ×2 — it drains the result max‑heap into the **reused** `HnswScratch.tN`/`tD` (`int[CAP]`, allocated **once** in `HnswScratch.init()`, mirroring `rN`/`rD`; `rSize ≤ ef ≤ CAP`). The only change is the buffer‑source line; the drain order, the `k` fill and the `-1` padding are untouched, so the output is **byte‑identical by construction** (Gate 1 confirmed `RecallHnsw` recall@5 96.89 % / approved‑agreement 99.90 % identical to Wave 5, `Rbh2Equiv` 0/3,000,000, both oracles byte‑exact). The production hot path (`search` → `top5Hnsw` → `searchLayer`/`takeTop5`) is therefore now **truly zero‑allocation per request** — Gate 2 (`AllocCheck`) measured **0 bytes / 100,000 queries (0.00 B/query)** vs the pre‑Wave‑6 baseline ≈400 B/query. The only remaining heap allocation is the **one‑time** dataset/graph bootstrap at startup. Wave 6 is an **optional** polish wave; the project was already technically complete at Wave 5 (behaviour, and therefore the `final_score`, are unchanged).
...
| The one `takeTop5` heap drain (§5) + no `LICENSE` | **done (6) — validated 2026‑05‑18** — `takeTop5` drains into reused `HnswScratch.tN`/`tD`; the hot path is now proven **zero‑alloc (0 B/query)**, behaviour **byte‑identical** (`RecallHnsw` 96.89 %/99.90 % identical to Wave 5); MIT `LICENSE` added at the repo root. Optional wave — the project was already complete at Wave 5 | 6 |
| Detection accuracy ~4393 vs leaderboard 6000 (int8+HNSW diverges from the exact round4 float ground truth) | **spec+tutorial ready (Wave 7, hand‑impl pending)** — reopens the project for the leaderboard top: "valid for submission" ≠ "competitive there"; p99 is already at the ceiling, so 100 % of the gap is accuracy (E=370, FP=61/FN=103, leaders E≈0). See `docs/superpowers/specs/2026-05-18-onda7-exact-accuracy-design.md` (commit `3a3cf01`) / `docs/TUTORIAL_EXACT.md`; **B3**: int16‑lossless (`×10000` represents round4 exactly) + exact `double` rerank (matches `data-generator/main.c`) + ambiguity escalation; target ~5900–6000. *Not implemented — design only; does not invalidate the Wave‑5 closure / "valid for submission".* | 7 |
